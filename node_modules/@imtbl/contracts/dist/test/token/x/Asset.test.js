...
describe("Asset", function () {
    it("Should be able to mint successfully with a valid blueprint", async function () {
        ...
        const Asset = await hardhat_1.ethers.getContractFactory("Asset");
        ...
        (0, chai_1.expect)(oo).to.equal(owner.address);
        const bp = await mintable.blueprints(tokenID);
        (0, chai_1.expect)(fromHex(bp)).to.equal(blueprint);
    });
    ...
    it("Should not be able to mint successfully with an invalid blueprint", async function () {
        ...
        const mintable = await Asset.deploy(o, name, symbol, imx);
        const blob = toHex(`:`);
        await (0, chai_1.expect)(mintable.mintFor(owner.address, 1, blob)).to.be.reverted;
    });
});
function toHex(str) {
    let result = "";
