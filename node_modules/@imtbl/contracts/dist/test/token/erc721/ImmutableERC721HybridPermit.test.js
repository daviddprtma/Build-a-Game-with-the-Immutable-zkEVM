...
describe("ImmutableERC721Permit", function () {
    ...
    let chainId;
    let eip1271Wallet;
    async function eoaSign(signer, spender, tokenId, nonce, deadline) {
        const typedData = {
            types: {
                Permit: [
        ...
        const signature = await signer._signTypedData(typedData.domain, { Permit: typedData.types.Permit }, typedData.message);
        return signature;
    }
    before(async function () {
        // Retrieve accounts
        [owner, user, minter, registrar, operator] = await hardhat_1.ethers.getSigners();
        // Get all required contracts
        ({ erc721, operatorAllowlist, eip1271Wallet } = await (0, DeployHybridFixtures_1.AllowlistFixture)(owner));
        // Set up roles
    ...
    describe("Interfaces", async function () {
        it("implements the erc4494 interface", async function () {
            (0, chai_1.expect)(await erc721.supportsInterface("0x5604e225")).to.equal(true);
        });
    });
    describe("EOA Permit", async function () {
        it("can use permits to approve spender on token minted by ID", async function () {
            ...
            (0, chai_1.expect)(nonce).to.be.equal(0);
            const operatorAddress = await operator.getAddress();
            const signature = await eoaSign(user, operatorAddress, 1, nonce, deadline);
            (0, chai_1.expect)(await erc721.getApproved(1)).to.not.equal(operatorAddress);
            console.log("operator: ", operatorAddress);
            await erc721.connect(operator).permit(operatorAddress, 1, deadline, signature);
            (0, chai_1.expect)(await erc721.getApproved(1)).to.be.equal(operatorAddress);
        });
        it("can use permits to approve spender on token minted by quantity", async function () {
            ...
            (0, chai_1.expect)(await erc721.getApproved(first)).to.not.equal(operatorAddress);
            await erc721.connect(operator).permit(operatorAddress, first, deadline, signature);
            (0, chai_1.expect)(await erc721.getApproved(first)).to.be.equal(operatorAddress);
        });
        it("reverts on permit if deadline has passed", async function () {
            ...
            await (0, chai_1.expect)(erc721.connect(operator).permit(operatorAddress, 2, deadline, signature)).to.be.revertedWith("PermitExpired");
            (0, chai_1.expect)(await erc721.getApproved(2)).to.not.equal(operatorAddress);
        });
        it("allows approved operators to create permits on behalf of token owner", async function () {
            ...
            await (0, chai_1.expect)(erc721.connect(operator).permit(operatorAddress, 3, deadline, signature)).to.be.revertedWith("InvalidSignature");
            (0, chai_1.expect)(await erc721.getApproved(3)).to.not.equal(operatorAddress);
            await erc721.connect(user).approve(ownerAddr, 3);
            await erc721.connect(operator).permit(operatorAddress, 3, deadline, signature);
            (0, chai_1.expect)(await erc721.getApproved(3)).to.be.equal(operatorAddress);
        });
        it("can not use a permit after a transfer due to bad nonce", async function () {
            ...
            await erc721
                .connect(user)["safeTransferFrom(address,address,uint256)"](await user.getAddress(), await owner.getAddress(), 4);
            nonce = await erc721.nonces(4);
            (0, chai_1.expect)(nonce).to.be.equal(1);
            await erc721
                .connect(owner)["safeTransferFrom(address,address,uint256)"](await owner.getAddress(), await user.getAddress(), 4);
            nonce = await erc721.nonces(4);
            (0, chai_1.expect)(nonce).to.be.equal(2);
            await (0, chai_1.expect)(erc721.connect(operator).permit(operatorAddress, 4, deadline, signature)).to.be.revertedWith("InvalidSignature");
        });
        it("can not use a permit after a transfer of token minted by id due to bad owner", async function () {
            ...
            const operatorAddress = await operator.getAddress();
            await erc721
                .connect(user)["safeTransferFrom(address,address,uint256)"](await user.getAddress(), await owner.getAddress(), 5);
            const nonce = await erc721.nonces(5);
            (0, chai_1.expect)(nonce).to.be.equal(1);
            const signature = await eoaSign(user, operatorAddress, 5, nonce, deadline);
            await (0, chai_1.expect)(erc721.connect(operator).permit(operatorAddress, 5, deadline, signature)).to.be.revertedWith("InvalidSignature");
        });
        it("can not use a permit after a transfer of token minted by quantity due to bad owner", async function () {
            ...
            await erc721
                .connect(user)["safeTransferFrom(address,address,uint256)"](await user.getAddress(), await owner.getAddress(), tokenId);
            const nonce = await erc721.nonces(tokenId);
            (0, chai_1.expect)(nonce).to.be.equal(1);
            const signature = await eoaSign(user, operatorAddress, tokenId, nonce, deadline);
            await (0, chai_1.expect)(erc721.connect(operator).permit(operatorAddress, tokenId, deadline, signature)).to.be.revertedWith("InvalidSignature");
        });
    });
    describe("Smart Contract Permit", async function () {
        it("can use permits to approve spender", async function () {
            ...
            (0, chai_1.expect)(await erc721.balanceOf(eip1271Wallet.address)).to.equal(1);
            const deadline = Math.round(Date.now() / 1000 + 24 * 60 * 60);
            const nonce = await erc721.nonces(6);
            (0, chai_1.expect)(nonce).to.be.equal(0);
            const operatorAddress = await operator.getAddress();
            const signature = await eoaSign(owner, operatorAddress, 6, nonce, deadline);
            (0, chai_1.expect)(await erc721.getApproved(6)).to.not.equal(operatorAddress);
            await erc721.connect(operator).permit(operatorAddress, 6, deadline, signature);
            (0, chai_1.expect)(await erc721.getApproved(6)).to.be.equal(operatorAddress);
        });
        it("does not allow approved operators to create permits on behalf of token owner", async function () {
            ...
            await (0, chai_1.expect)(erc721.connect(operator).permit(operatorAddress, tokenId, deadline, signature)).to.be.revertedWith("InvalidSignature");
            (0, chai_1.expect)(await erc721.getApproved(tokenId)).to.not.equal(operatorAddress);
